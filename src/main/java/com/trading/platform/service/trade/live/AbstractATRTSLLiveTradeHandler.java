...
public abstract class AbstractATRTSLLiveTradeHandler extends AbstractATRTSLTradeHandler {

	protected AbstractATRTSLLiveTradeHandler(Job job, TradeInfo tradeInfo, TradeManager tradeManager,
	...
	public Order placeMarketBuyOrder() {
		try {
			Order order = getSession().placeMarketBuyOrder(getTradeInfo().getSignal().getOptionSymbol(),
					getQuantity());
